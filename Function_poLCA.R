...
for (var in xtab_var_rows){
  ...
  num_msg_cnt <- vector()
  num_msg_pct <- vector()
  for(col in seq(1,ncol(xtab_table_iter)-1,1)){
    num_msg_cnt[col] <- sum( as.numeric( XTab_Data[,3][XTab_Data[,2]==col & is.na(XTab_Data[,var])] ))
    num_msg_pct[col] <- num_msg_cnt[col] / SEG_N_WTD[col]
  }
